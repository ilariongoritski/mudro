...
func TestBuildCommentAuthor(t *testing.T) {
	if got := buildCommentAuthor(csvCommentRow{Sender: "Иларион"}, nil); got != "Иларион" {
		t.Fatalf("expected sender name, got %#v", got)
	}
	if got := buildCommentAuthor(csvCommentRow{SenderID: "424169621"}, nil); got != "Участник #424169621" {
		t.Fatalf("expected sender id fallback, got %#v", got)
	}
	if got := buildCommentAuthor(csvCommentRow{SenderID: "424169621"}, map[string]string{"user424169621": "Алина"}); got != "Алина" {
		t.Fatalf("expected sender name from json map, got %#v", got)
	}
	if got := buildCommentAuthor(csvCommentRow{SenderID: "-1"}, nil); got != nil {
