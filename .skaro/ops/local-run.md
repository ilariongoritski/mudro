...
- backend and `make` commands run through WSL at `/mnt/d/mudr/mudro11-automation`
...
For this worktree, the minimal safe Skaro loop is:

1. `git status --short`
2. `Skaro: UI`
3. `Skaro: Open Dashboard`
4. `Skaro: Status`
5. Update the active task `spec.md` and `plan.md` before substantial changes
6. After implementation, run `Skaro: Validate`
7. Sync outcome into `.codex/state.md` and `.codex/done.md`
