...
func main() {
	...
	for _, id := range ordered {
		...
		if err := upsertComment(ctx, pool, postID, row, parentCommentID); err != nil {
...
func upsertComment(ctx context.Context, pool *pgxpool.Pool, postID int64, row csvCommentRow, parentCommentID *string) error {
...
on conflict (source, source_comment_id) do update set
  ...
  updated_at = now()
returning id
`, postID, "tg", strconv.FormatInt(row.MessageID, 10), parentCommentID, parentRowID, buildCommentAuthor(row), row.Date, text, reactions, nil).Scan(&commentID)
	if err != nil {
		return err
	}
...
func buildCommentAuthor(row csvCommentRow) any {
