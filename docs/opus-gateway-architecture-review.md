...
3. Тесты завязаны на локальный путь разработчика.
   В `tools/opus-gateway/test/server.test.ts` используется жёсткий `repoRoot` вида `D:\\mudr\\mudro11`, из-за чего тестовый контур не является переносимым и будет ломаться вне этой машины.
