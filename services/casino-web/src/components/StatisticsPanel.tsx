...
export function StatisticsPanel() {
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      const totalBet = historyData.reduce((sum, s) => sum + s.bet, 0);
      const totalWin = historyData.reduce((sum, s) => sum + s.win, 0);
      const winRate = totalSpins > 0 ? (historyData.filter(s => s.win > 0).length / totalSpins) * 100 : 0;
      const biggestWin = historyData.length > 0 ? Math.max(...historyData.map(s => s.win)) : 0;
    ...
    setLoading(false);
  }, [isLoggedIn, loadHistory, history]);
