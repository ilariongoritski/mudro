...
export function ControlPanel() {
  ...
  const runSpin = async (): Promise<boolean> => {
    ...
    try {
      ...
      applyServerSpin({
        ...
        sweetBonanza: result.sweet_bonanza,
        freeSpinsBalance: result.free_spins_balance,
  ...
  const waitForAnimationEnd = () => new Promise<void>((resolve) => {
    const deadline = Date.now() + 45_000;
    const timer = window.setInterval(() => {
      const currentPhase = useSlot.getState().phase;
      if (currentPhase === "idle" || currentPhase === "ended" || Date.now() >= deadline) {
